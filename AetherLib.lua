...
--// Designed with Ethereal Glassmorphism Aesthetic
...
local function PlaySound(soundId, volume)
    ...
    local sound = Create("Sound", {SoundId = soundId, Volume = volume or 0.3, Parent = game:GetService("SoundService")})
    sound:Play()
...
local function CreateParticles(parent)
    local particleContainer = Create("Frame", {Name = "Particles", Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, ClipsDescendants = true, Parent = parent, ZIndex = 0})
    for i = 1, 15 do
        ...
        task.spawn(function()
            while particle and particle.Parent do
                local newY, newX = math.random(-50, 50), math.random(-30, 30)
                Tween(particle, {Position = UDim2.new(particle.Position.X.Scale, particle.Position.X.Offset + newX, particle.Position.Y.Scale, particle.Position.Y.Offset + newY), BackgroundTransparency = math.random(3, 9) / 10}, math.random(3, 6), Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
...
function AetherLib:Notify(data)
    ...
    if not NotificationActive then self:ProcessNotificationQueue() end
end
...
function AetherLib:ProcessNotificationQueue()
    ...
    local screenGui = gui:FindFirstChild("AetherNotifications") or Create("ScreenGui", {Name = "AetherNotifications", Parent = gui, ResetOnSpawn = false, ZIndexBehavior = Enum.ZIndexBehavior.Sibling})
    local notifFrame = Create("Frame", {Name = "Notification", Size = UDim2.new(0, 320, 0, 90), Position = UDim2.new(1, 20, 0.85, 0), BackgroundColor3 = AetherLib.Config.Theme.Surface, BackgroundTransparency = 0.15, BorderSizePixel = 0, Parent = screenGui})
    ...
    Create("UIGradient", {Color = ColorSequence.new({ColorSequenceKeypoint.new(0, AetherLib.Config.Theme.Background), ColorSequenceKeypoint.new(1, AetherLib.Config.Theme.Surface)}), Rotation = 45, Parent = notifFrame})
    ...
    task.delay(duration, function()
        Tween(notifFrame, {Position = UDim2.new(1, 20, 0.85, 0)}, 0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.In, function() notifFrame:Destroy(); self:ProcessNotificationQueue() end)
    end)
end
...
function AetherLib:CreateWindow(data)
    ...
    local mainFrame = Create("Frame", {Name = "MainFrame", Size = UDim2.new(0, 0, 0, 0), Position = UDim2.new(0.5, 0, 0.5, 0), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = AetherLib.Config.Theme.Background, BackgroundTransparency = 0.05, BorderSizePixel = 0, Parent = screenGui, ClipsDescendants = true})
    ...
    Create("UIGradient", {Color = ColorSequence.new({ColorSequenceKeypoint.new(0, AetherLib.Config.Theme.Background), ColorSequenceKeypoint.new(0.5, AetherLib.Config.Theme.Surface), ColorSequenceKeypoint.new(1, AetherLib.Config.Theme.Background)}), Rotation = 135, Parent = mainFrame})
    ...
    closeBtn.MouseButton1Click:Connect(function() PlaySound(AetherLib.Config.Sounds.Click, 0.2); Tween(mainFrame, {Size = UDim2.new(0, 0, 0, 0)}, 0.4, Enum.EasingStyle.Back, Enum.EasingDirection.In, function() screenGui:Destroy() end) end)
    ...
    function Window:SelectTab(tabName)
        for name, tab in pairs(Window.Tabs) do
            if name == tabName then
                ...
                tab.Button:FindFirstChild("UIStroke").Enabled = true
                Tween(tab.Button:FindFirstChild("UIStroke"), {Transparency = 0.3}, 0.2)
                Tween(tab.Button.Icon, {ImageColor3 = AetherLib.Config.Theme.Accent}, 0.2)
                Tween(tab.Button.Text, {TextColor3 = AetherLib.Config.Theme.Accent}, 0.2)
                Window.CurrentTab = tabName
            else
                tab.Content.Visible = false
                Tween(tab.Button, {BackgroundTransparency = 0.7}, 0.2)
                tab.Button:FindFirstChild("UIStroke").Enabled = false
                Tween(tab.Button.Icon, {ImageColor3 = AetherLib.Config.Theme.SubText}, 0.2)
                Tween(tab.Button.Text, {TextColor3 = AetherLib.Config.Theme.SubText}, 0.2)
            end
        end
    end
    ...
    function Window:CreateTab(tabData)
        ...
        Create("ImageLabel", {Name = "Icon", Size = UDim2.new(0, 18, 0, 18), Position = UDim2.new(0, 10, 0.5, -9), BackgroundTransparency = 1, Image = tabIcon ~= "" and tabIcon or "rbxassetid://7733965386", ImageColor3 = AetherLib.Config.Theme.SubText, Parent = tabBtn, ZIndex = 4})
        Create("TextLabel", {Name = "Text", Size = UDim2.new(1, -40, 1, 0), Position = UDim2.new(0, 34, 0, 0), BackgroundTransparency = 1, Text = tabName, TextColor3 = AetherLib.Config.Theme.SubText, TextSize = 14, Font = Enum.Font.GothamSemibold, TextXAlignment = Enum.TextXAlignment.Left, Parent = tabBtn, ZIndex = 4})
        ...
        tabBtn.MouseButton1Click:Connect(function() PlaySound(AetherLib.Config.Sounds.Click, 0.15); Window:SelectTab(tabName) end)
        tabBtn.MouseEnter:Connect(function() if Window.CurrentTab ~= tabName then Tween(tabBtn, {BackgroundTransparency = 0.4}, 0.2); Tween(tabBtn.Icon, {ImageColor3 = AetherLib.Config.Theme.Text}, 0.2); Tween(tabBtn.Text, {TextColor3 = AetherLib.Config.Theme.Text}, 0.2) end end)
        tabBtn.MouseLeave:Connect(function() if Window.CurrentTab ~= tabName then Tween(tabBtn, {BackgroundTransparency = 0.7}, 0.2); Tween(tabBtn.Icon, {ImageColor3 = AetherLib.Config.Theme.SubText}, 0.2); Tween(tabBtn.Text, {TextColor3 = AetherLib.Config.Theme.SubText}, 0.2) end end)
        ...
        function Tab:CreateButton(btnData)
            ...
            if btnDescription ~= "" then Create("TextLabel", {Name = "Description", Size = UDim2.new(0.7, 0, 0, 16), Position = UDim2.new(0, 12, 0, 28), BackgroundTransparency = 1, Text = btnDescription, TextColor3 = AetherLib.Config.Theme.SubText, TextSize = 11, Font = Enum.Font.Gotham, TextXAlignment = Enum.TextXAlignment.Left, Parent = btnFrame, ZIndex = 4}) end
            ...
            clickBtn.MouseEnter:Connect(function() Tween(btnFrame, {BackgroundTransparency = 0.2}, 0.2); Tween(btnStroke, {Transparency = 0.1}, 0.2); Tween(actionIcon, {ImageColor3 = AetherLib.Config.Theme.Text}, 0.2) end)
            clickBtn.MouseLeave:Connect(function() Tween(btnFrame, {BackgroundTransparency = 0.5}, 0.2); Tween(btnStroke, {Transparency = 0.4}, 0.2); Tween(actionIcon, {ImageColor3 = AetherLib.Config.Theme.Accent}, 0.2) end)
            clickBtn.MouseButton1Click:Connect(function() PlaySound(AetherLib.Config.Sounds.Click, 0.15); Tween(btnFrame, {Size = UDim2.new(0.98, 0, 0, btnDescription ~= "" and 55 or 40)}, 0.1, Enum.EasingStyle.Quart, Enum.EasingDirection.Out, function() Tween(btnFrame, {Size = UDim2.new(1, 0, 0, btnDescription ~= "" and 55 or 40)}, 0.15, Enum.EasingStyle.Back, Enum.EasingDirection.Out) end); btnCallback() end)
            return btnFrame
        end
        ...
        function Tab:CreateToggle(toggleData)
            ...
            if toggleDescription ~= "" then Create("TextLabel", {Name = "Description", Size = UDim2.new(0.6, 0, 0, 16), Position = UDim2.new(0, 12, 0, 28), BackgroundTransparency = 1, Text = toggleDescription, TextColor3 = AetherLib.Config.Theme.SubText, TextSize = 11, Font = Enum.Font.Gotham, TextXAlignment = Enum.TextXAlignment.Left, Parent = toggleFrame, ZIndex = 4}) end
            ...
            local function UpdateToggle() if state then Tween(toggleBackground, {BackgroundColor3 = AetherLib.Config.Theme.Success}, 0.25); Tween(toggleCircle, {Position = UDim2.new(1, -21, 0.5, -9)}, 0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out); Tween(toggleStroke2, {Color = AetherLib.Config.Theme.Success, Transparency = 0.2}, 0.25) else Tween(toggleBackground, {BackgroundColor3 = AetherLib.Config.Theme.Background}, 0.25); Tween(toggleCircle, {Position = UDim2.new(0, 3, 0.5, -9)}, 0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out); Tween(toggleStroke2, {Color = AetherLib.Config.Theme.SubText, Transparency = 0.5}, 0.25) end end
            if state then UpdateToggle() end
            toggleBtn.MouseButton1Click:Connect(function() PlaySound(AetherLib.Config.Sounds.Toggle, 0.12); state = not state; UpdateToggle(); toggleCallback(state) end)
            toggleBtn.MouseEnter:Connect(function() Tween(toggleFrame, {BackgroundTransparency = 0.2}, 0.2); Tween(toggleStroke, {Transparency = 0.1}, 0.2) end)
            toggleBtn.MouseLeave:Connect(function() Tween(toggleFrame, {BackgroundTransparency = 0.5}, 0.2); Tween(toggleStroke, {Transparency = 0.4}, 0.2) end)
            local ToggleObj = {}
        ...
        function Tab:CreateSlider(sliderData)
            ...
            Create("UICorner", {CornerRadius = UDim.new(1, 0), Parent = sliderBarFill})
            ...
            local function UpdateSlider(input)
                ...
                sliderBarFill.Size = UDim2.new(scale, 0, 1, 0)
                sliderKnob.Position = UDim2.new(scale, -9, 0.5, -9)
                valueLabel.Text = tostring(val) .. suffix
                callback(val)
                return val
            end
            sliderBarBg.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then dragging = true; UpdateSlider(input); Tween(sliderKnob, {Size = UDim2.new(0, 22, 0, 22)}, 0.15) end end)
            sliderKnob.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then dragging = true; Tween(sliderKnob, {Size = UDim2.new(0, 22, 0, 22)}, 0.15) end end)
            ...
            UserInputService.InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then dragging = false; Tween(sliderKnob, {Size = UDim2.new(0, 18, 0, 18)}, 0.15) end end)
            sliderBarBg.MouseEnter:Connect(function() Tween(sliderFrame, {BackgroundTransparency = 0.2}, 0.2); Tween(sliderStroke, {Transparency = 0.1}, 0.2) end)
            sliderBarBg.MouseLeave:Connect(function() if not dragging then Tween(sliderFrame, {BackgroundTransparency = 0.5}, 0.2); Tween(sliderStroke, {Transparency = 0.4}, 0.2) end end)
            local SliderObj = {}
            function SliderObj:Set(val) val = math.clamp(math.floor(val / increment + 0.5) * increment, min, max); local scale = (val - min) / (max - min); sliderBarFill.Size = UDim2.new(scale, 0, 1, 0); sliderKnob.Position = UDim2.new(scale, -9, 0.5, -9); valueLabel.Text = tostring(val) .. suffix; callback(val) end
        ...
        function Tab:CreateDropdown(dropData)
            ...
            local function BuildOptions()
                ...
                for _, option in ipairs(options) do
                    ...
                    optBtn.MouseButton1Click:Connect(function() PlaySound(AetherLib.Config.Sounds.Click, 0.12); selectedLabel.Text = option; callback(option); open = false; Tween(dropFrame, {Size = UDim2.new(1, 0, 0, 42)}, 0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out); Tween(arrowIcon, {Rotation = 90}, 0.3); optionsFrame.Visible = false end)
                end
                local totalHeight = #options * 36 + 8
                optionsFrame.Size = UDim2.new(1, -16, 0, math.min(totalHeight, 180))
            end
            BuildOptions()
            dropBtn.MouseButton1Click:Connect(function() PlaySound(AetherLib.Config.Sounds.Click, 0.12); open = not open; if open then optionsFrame.Visible = true; Tween(dropFrame, {Size = UDim2.new(1, 0, 0, math.min(46 + optionsFrame.Size.Y.Offset + 8, 240))}, 0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out); Tween(arrowIcon, {Rotation = -90}, 0.3) else Tween(dropFrame, {Size = UDim2.new(1, 0, 0, 42)}, 0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out); Tween(arrowIcon, {Rotation = 90}, 0.3); task.delay(0.3, function() optionsFrame.Visible = false end) end end)
            dropBtn.MouseEnter:Connect(function() Tween(dropFrame, {BackgroundTransparency = 0.2}, 0.2); Tween(dropStroke, {Transparency = 0.1}, 0.2) end)
            dropBtn.MouseLeave:Connect(function() Tween(dropFrame, {BackgroundTransparency = 0.5}, 0.2); Tween(dropStroke, {Transparency = 0.4}, 0.2) end)
            local DropObj = {}
            function DropObj:Set(val) if table.find(options, val) then selectedLabel.Text = val; callback(val) end end
            function DropObj:Refresh(newOptions) options = newOptions; BuildOptions() end
            function DropObj:Get() return selectedLabel.Text end
            return DropObj
        end
        ...
        function Tab:CreateInput(inputData)
            ...
            textBox.Focused:Connect(function() PlaySound(AetherLib.Config.Sounds.Hover, 0.08); Tween(textBoxStroke, {Transparency = 0.1, Color = AetherLib.Config.Theme.Accent}, 0.2); Tween(textBox, {BackgroundTransparency = 0.1}, 0.2) end)
            textBox.FocusLost:Connect(function() Tween(textBoxStroke, {Transparency = 0.6, Color = AetherLib.Config.Theme.Primary}, 0.2); Tween(textBox, {BackgroundTransparency = 0.3}, 0.2); callback(textBox.Text) end)
        ...
        function Tab:CreateLabel(labelData)
            ...
            local LabelObj = {}
            function LabelObj:Set(text) textLabel.Text = text end
            function LabelObj:SetColor(color) textLabel.TextColor3 = color end
            return LabelObj
        end
        ...
        function Tab:CreateKeybind(bindData)
            ...
            keyBtn.MouseButton1Click:Connect(function() PlaySound(AetherLib.Config.Sounds.Click, 0.12); listening = true; keyBtn.Text = "..."; Tween(keyBtn, {BackgroundColor3 = AetherLib.Config.Theme.Primary}, 0.2) end)
            UserInputService.InputBegan:Connect(function(input, gameProcessed)
                if listening and not gameProcessed then
                    if input.UserInputType == Enum.UserInputType.Keyboard then
                        listening = false; currentKey = input.KeyCode.Name; keyBtn.Text = currentKey; Tween(keyBtn, {BackgroundColor3 = AetherLib.Config.Theme.Background}, 0.2); callback(currentKey)
                    end
                elseif input.KeyCode.Name == currentKey and not gameProcessed then callback(currentKey) end
            end)
            keyBtn.MouseEnter:Connect(function() Tween(bindFrame, {BackgroundTransparency = 0.2}, 0.2); Tween(bindStroke, {Transparency = 0.1}, 0.2) end)
            keyBtn.MouseLeave:Connect(function() Tween(bindFrame, {BackgroundTransparency = 0.5}, 0.2); Tween(bindStroke, {Transparency = 0.4}, 0.2) end)
            local BindObj = {}
        ...
        function Tab:CreateColorPicker(pickerData)
            ...
            pickerBtn.MouseButton1Click:Connect(function() PlaySound(AetherLib.Config.Sounds.Click, 0.12); local r, g, b = math.floor(currentColor.R * 255), math.floor(currentColor.G * 255), math.floor(currentColor.B * 255); r = math.clamp(r + math.random(-30, 30), 0, 255); g = math.clamp(g + math.random(-30, 30), 0, 255); b = math.clamp(b + math.random(-30, 30), 0, 255); currentColor = Color3.fromRGB(r, g, b); Tween(colorPreview, {BackgroundColor3 = currentColor}, 0.3); callback(currentColor) end)
            pickerBtn.MouseEnter:Connect(function() Tween(pickerFrame, {BackgroundTransparency = 0.2}, 0.2); Tween(pickerStroke, {Transparency = 0.1}, 0.2) end)
            pickerBtn.MouseLeave:Connect(function() Tween(pickerFrame, {BackgroundTransparency = 0.5}, 0.2); Tween(pickerStroke, {Transparency = 0.4}, 0.2) end)
            local PickerObj = {}
            function PickerObj:Set(color) currentColor = color; Tween(colorPreview, {BackgroundColor3 = color}, 0.3); callback(color) end
